...
schedule function event_games:cow_sorting/kill_reset_cows 8t
scoreboard players set @e[tag=cow_sorting_center] evg_tmp 15
execute as @e[tag=cow_sorting_center] at @s run function event_games:cow_sorting/summon_cows
tag @a[tag=cow_sorting_player] remove cow_sorting_player
scoreboard players reset @e[tag=cow_sorting_center] evg_tmp
execute as @e[tag=cow_sorting_center] at @s unless block ~ ~-1 ~ minecraft:red_concrete run kill @s

#cancel timers
schedule clear event_games:countdowns/buzzer_start
schedule clear event_games:cow_sorting/calculate_scores
...
scoreboard players set cow_sorting_active evg_persistent 0